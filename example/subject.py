...
subjects = BIDSLayout(root=path/'rawdata', validate=False, index_metadata=False).get_subjects()
...
for row in record.values:
    ...
    for sub in subjects:
        files = sorted(wb_ppi.glob(f'sub-{sub}/sub-{sub}_task-h?_atlas-{row[0]}_strat-{row[1]}_ppi.npy'))
        ppi_vect = np.array([np.load(file) for file in files])
        
        pre, post = ppi_vect[:2,].mean(axis=0), ppi_vect[2:,].mean(axis=0)
