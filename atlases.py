...
class Atlas:
    ...
    def get_data(self):
        ...
        if self.title == 'yeo2011':
            ...
            fetcher = datasets.fetch_atlas_schaefer_2018(n_rois=100, yeo_networks=7, resolution_mm=2)
            maps = fetcher.maps
            ...
            maps = fetcher.maps 
            df = pd.DataFrame([label[1] for label in fetcher.labels], columns=['labels'])
            probabilistic = True
            ...
            fetcher = datasets.fetch_atlas_harvard_oxford('cort-maxprob-thr25-2mm', symmetric_split=True)
            maps = fetcher.maps
    ...
    def get_masker(self, maps, probabilistic, derivatives, smooth_fwhm):
        ...
        if probabilistic:
            ...
            return input_data.NiftiLabelsMasker(labels_img=maps, strategy='mean', **kwargs)