...
def get_args():
    ...
    assert args.strategy in strategies.keys(), f'{args.strategy} is not a valid strategy'
...
class Data:
    def __init__(self, fmriprep, strategy):
        ...
        assert len(self.preprocs)==len(self.confounds), \
            'missings fmriprep files'
        print(f'found {len(self.preprocs)} images')

    def get_preprocs(self):
        preprocs = self.fmriprep.glob('**/sub-*_task-h*_space-MNI152NLin6Asym_res-2_desc-preproc_bold.nii.gz')
        return sorted([str(preproc) for preproc in preprocs])

    def get_events(self):
        events = sorted(self.fmriprep.parents[1].glob('rawdata/**/sub-*_task-h*events.tsv'))
        return [pd.read_csv(event, sep='\t', usecols=['onset', 'duration', 'trial_type']) for event in events]
...
def fit_ppi(ts_rois, dm, model):
    ...
    for roi in ts_rois.T:
        ...
        ppi_vect.append(model.coef_[:,-1])  # beta coefficients corresponding to ppi regressor
...
def main():  
    ...
    masker = atlas.get_masker(atlas.maps, atlas.probabilistic, derivatives, args.smooth_fwhm)
